...
export const sticksSlice = createSlice({
  ...
  reducers: {
    ...
    deleteStickFromTrash: (
      state: initialStateType,
      action: sticksActionType,
    ) => {
      state.trash.sticks = state.trash.sticks.filter(
        (stick) => stick !== action.payload.stick,
      );
